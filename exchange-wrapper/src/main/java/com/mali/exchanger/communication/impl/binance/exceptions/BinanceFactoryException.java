package com.mali.exchanger.communication.impl.binance.exceptions;

import com.mali.validator.ServiceErrorItem;

import java.util.List;

public class BinanceFactoryException extends  Exception {

    private List<ServiceErrorItem> errors;

    public BinanceFactoryException(String message) {
        super(message);
    }

    public BinanceFactoryException(String message, List<ServiceErrorItem> errors) {
        super(message);
        this.errors = errors;
    }

    public List<ServiceErrorItem> getErrors() {
        return errors;
    }

    public void setErrors(List<ServiceErrorItem> errors) {
        this.errors = errors;
    }
}
